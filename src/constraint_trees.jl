...
ConstraintTrees.substitute(x::ParameterLinearValue, y::Vector{FastDifferentiation.Number}) =
    ConstraintTrees.sum(
        (idx == 0 ? x.weights[i] : x.weights[i] * y[idx] for (i, idx) in enumerate(x.idxs)),
        init = FastDifferentiation.Number(0.0),
    )

ConstraintTrees.substitute(x::ParameterQuadraticValue, y::Vector{FastDifferentiation.Number}) =
    ConstraintTrees.sum(
        (
