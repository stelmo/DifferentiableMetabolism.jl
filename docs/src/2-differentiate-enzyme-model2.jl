...
ec_model = X.enzyme_constrained_flux_balance_constraints( # kinetic model
    model;
    reaction_isozymes,
...
ec_solution = D.optimized_constraints_with_parameters(
    ec_model,
    parameter_values;
    objective = ec_model.objective.value,
...
sens, vids, ps = D.differentiate_model(
    ec_model,
    ec_solution.tree;
    parameter_values,
    optimizer = T.Optimizer,
...
param_idxs = findall(x -> x in subset_ids, ps)
param_ids = ps[param_idxs]
