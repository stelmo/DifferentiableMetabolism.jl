...
plv9 = ParameterLinearValue([1, 3], [4,5]) #src
plv10 = plv8 + plv9 #src
...
@test all(pqv1.idxs[1] .== (1,1)) && all(pqv1.weights .== Num(1)) #src
...
pqv18 = ParameterQuadraticValue([(1, 1), (2, 2)], [1, 3]) #src
pqv19 = pqv18 + pqv1
...
pr11 = ParameterQuadraticValue([(1, 1)], [2,]) - ConstraintTrees.QuadraticValue([(1, 1)], [1,]) #src
...
@test all(pr12.idxs .== [(0,1), (1,1),(0,2)]) && all(pr12.weights .== [Num(1), Num(2),Num(2)]) #src
